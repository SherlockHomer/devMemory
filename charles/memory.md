...
### 不止抓包看数据
1. 会话记录
    * "File-Save Session"：保存记录到本地，用于API校对（听说后台喜欢改接口？）  
    * *如果停止记录，请求还是会通过Charles，但不会在记录面板上看到*
  
2. 聚焦（View-Structure-foucs）
    * focus 聚焦主机，当然也可以使用记录面板的filter 过滤你要的ip
...
4. 断点😎
    ...
    * 断点上修改request、response的数据，就能在App上修改数据，但是要快！免得算你网络超时
...
9. Map local Tool（映射本地站点，岂不是可以mock！）
    ...
    * 如果接口映射成目录，映射到本地的content-type会是text/plain，即使Local path写上 *.json也匹配不了，是charles的不足？*不，看rewrite！*
